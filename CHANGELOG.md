...
- **Per-session queue — `queueMode: "followup"` for shared topics** (#297 follow-up, default `"off"`): a Zulip topic is **one session**, so everyone in it shares one conversation and the bot works one request at a time there. The host's default for a message that arrives mid-run is `steer` — it is injected *into* the running turn, so in a shared room teammate B's message can redirect teammate A's work. The host can be told to wait (`messages.queue.mode: "followup"`), but the per-channel form is unusable here: `messages.queue.byChannel.zulip` is rejected as `Unrecognized key: "zulip"` (only known/bundled channel ids are accepted), a global mode would also change other channels, and a per-message override is not exposed to channel plugins (`queueModeOverride` exists only on the host's internal/gateway chat-send path). So the plugin queues **before** handing the message over, which keeps the change Zulip-only and means the host never sees two concurrent turns for one session.
  - A message arriving while a run is active for that topic (or DM) waits its turn; **separate topics are unaffected** and still run in parallel (different sessions).
  - The waiting message is marked with a **⏳ reaction** (`reactions.onQueued`, default `hourglass`) — Zulip has no "queued input" surface, so this is what makes the wait legible. 👀 still means received, ✅ still means finished.
  - `queueCap` (default 20) bounds the wait; past the cap a message is dispatched **immediately rather than dropped**, and the overflow is logged.
  - Each transition is audit-logged (`message_queued` with the message id and the number waiting, `message_dequeued` with the message id): the ⏳ is transient by design and plugin logs do not surface on every host, so the audit file is what proves the queue engaged.
  - Default `"off"`, so existing installs and other channels are completely unaffected.
...
- **A trace stayed "Working" forever when a run was killed by a restart** (#293, found in the field): a trace can only be finalized by the process that created it, so a deploy, crash or OOM landing mid-run left `**Working** — …` frozen in the topic permanently — the one case where the epic's "no trace is left permanently in progress" did not hold (observed live on y6 when a deploy restarted the gateway during a run). In-flight traces are now persisted to `{dataDir}/zulip_traces_{accountId}.json` and `recoverInterruptedTraces()` collapses them to `⚪ **Cancelled** — run interrupted by a gateway restart` at the next start, recorded as an `activity_trace_recovered` audit event (`activity_trace_recovery_failed` with the reason if the edit itself is refused — Zulip stops accepting edits to a message once the realm's edit limit passes, so a restart long after the run cannot be recovered). `stop()` persists rather than clears so a graceful shutdown is covered too; a failed recovery edit is logged while the file is still cleared, so it cannot repeat on every start.
- **Reaction triggers could not fire at all on a host with a persisted event queue** (#297, found in the field): `/register` fixes `event_types` for a queue's whole lifetime, and the queue is deliberately reused across restarts, but the persisted metadata never recorded which event types it had been registered for — so a queue registered days earlier for `["message"]` kept being reused after `reaction` was added to the request, and the events were simply never delivered. No error, no log, just silence. The metadata now records `eventTypes` and a persisted queue is only reused while the needed set matches (legacy metadata counts as `["message"]`, what it actually requested, so upgrades do not churn queues).
- **Reaction triggers were silently skipped in streams the bot was not subscribed to** (#297, found in the field): Zulip delivers `reaction` events only to *subscribers* of a stream, while `message` events arrive anyway because the queue uses `all_public_streams` — so an unsubscribed stream looks healthy and the trigger just never fires. The monitor now checks the bot's subscriptions at startup and warns with the missing streams (or, with `streams: ["*"]`, prints the subscribed list since `"*"` cannot be verified), and the README documents the requirement.
- **Reaction triggers built an un-authorizable sender** (#297, found in the field): a live `reaction` event carries `user_id` only — there is no `user` object — so the synthetic turn had `sender_email: null` and `handleMessage` derived its sender from the numeric id, which every email-based allowlist rejected (`groupPolicy: allowlist`) as an invisible policy drop. The plugin now resolves the reacting user's email via `fetchZulipUser` before building the turn, and drops the trigger **with a warning** if it cannot, instead of failing closed in silence. The triggered run's trace line also renders as `reaction :+1: — <instruction>` instead of echoing the internal `[Zulip reaction] …` envelope into the room.
